...
(`packages/plugin-deploy/src/abi-compat.ts`) re-parses both the stored snapshot and the new
ABI before diffing, compares transition/view inputs **positionally** (by mode + type, not
name), and no longer enforces `implements` or `const_parameters` (both removed in 4.2). This
is what lets a legitimate Leo 4.1 → 4.2 upgrade of an unchanged signature pass.

**`abiHash` is a recorded fingerprint, not a gate.** `computeAbiHash`
(`packages/leo-compiler/src/abi-hash.ts`) is written into deploy/upgrade records but is never
compared as a correctness check. It drifts cosmetically across the 4.1 → 4.2 cutover (names
synthesized, modes/self-refs/implements/const-params canonicalized); this is harmless.
