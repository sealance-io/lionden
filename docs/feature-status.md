...
| `upgrade` task (thin): compile v2 → build upgrade tx → broadcast → record; returns `{ programId, txId, blockHeight }`; selects the signing key from `namedAccounts.admin` when set (selection only, no address-match validation); does NOT validate ABI compatibility, constructor immutability, edition, or admin identity — Leo's built-in tooling owns upgrade correctness | `packages/plugin-deploy/src/upgrade-task.ts` |
| `export` task: per-network bundle to `deployments/_exports/<network>.json` or `--out <path>`; writes even in ephemeral mode; emits per-program ABI (`ExportedProgram` carries `programId`, `abi`, `txId`, `status`) | `packages/plugin-deploy/src/index.ts` |
...
| 4 | `@admin` upgrade (thin: compile v2 → build → broadcast → record); history append; mapping survives upgrade; post-upgrade execution via `ctx.raw.execute()` |
...
- **Pending *upgrade* recovery**: probe 5 covers pending `action: "deploy"`; the pending-upgrade variant is not probed.
...
- **ABI snapshot fallback to artifact**: the missing-snapshot fallback to `artifacts/<programId>/abi.json` (used by `export`) is not exercised by a probe.
...
### Devnode lifecycle control granularity

Today `@lionden/testing`'s `setup()` is gated by `config.testing.autoStartDevnode` (default `true`). Devnode lifecycle is effectively coupled to whoever calls `setup()` first in a test process, with teardown driven by `ctx.teardown()` and fixture caching via `loadFixture`. The practical granularity is "per process" — Vitest runs each test file in its own forked worker, so the effective unit is "per file".

**Resolved: external lifecycle is supported through the `testing` hook category.** Users who need to run against a network LionDen does not manage — a containerized multi-validator devnet, a shared staging chain — declare it as an `http` network and own its lifecycle from a `suiteSetup` / `suiteTeardown` handler on a local plugin. The `test` task dispatches `suiteSetup` in the parent process before Vitest forks, so a dynamically discovered endpoint published to `process.env` is inherited by every worker. No `--no-managed-devnode` flag and no Vitest `globalSetup` passthrough is needed. This is the pattern that replaced the Testcontainers `vitest.global-setup.ts` in `compliant-transfer-aleo`. See [`testing.md`](testing.md#bring-your-own-network-lifecycle) for the full recipe, including the precompile-then-`--no-compile` requirement and the one-chain-per-file caveat.

Still open for post-V1 DX:
...
- **Cross-file state sharing**: with the current per-file devnode, deployments don't persist across test files (each gets a fresh devnode). Some test suites want shared on-chain state across files (sequential, ordered execution) — that's not a supported pattern today. The BYO-network recipe gets there incidentally (one external chain shared by every file in an invocation), but it does not give LionDen-managed ordering guarantees.
- **Retargeting compile from a hook**: a hook can publish an endpoint for workers, but not for the parent's own `compile` — the CLI resolves config and builds the LRE at boot, and `compilePipeline` reads `lre.config`. Making a dynamically mapped endpoint visible to compile needs an LRE-rebind API, which is a separate unsolved design question.
- **BYO-lifecycle ergonomics**: the recipe currently requires explicit `.ts` module specifiers for anything the config imports (workers load the config with Node's own TypeScript loader, which does not rewrite `.js` → `.ts`), and therefore `allowImportingTsExtensions` — which a stock `create-lionden` project, on `NodeNext` with `declaration: true`, cannot set without also switching to `noEmit`/`emitDeclarationOnly`. Normalizing config-relative imports in the loader would remove that requirement.

The items above are open design questions. The item below is a concrete defect with a known consumer workaround:

- **Consensus-height priming is devnode-only.** `initConsensusHeights()` is called only when `connection.type === "devnode"`, but a *test devnet* reached over `http` runs the same compressed activation schedule. Without priming, such a chain rejects LionDen-built deployments with `missing program checksum` at the first deploy — reproduced by A/B against `aleo-devnet:v4.3.1-v4.8.1`, where the only difference between a failing and a passing run was calling the exported `initConsensusHeights()` from the project config. (The precise SDK-internal mechanism is inferred: unprimed, the SDK appears to resolve the consensus version from the production height table and omit the program checksum. Causality is measured; that explanation is not.) Consumers can work around it from their config. The open part is deciding when LionDen should prime for `http` — always, or only when the chain reports a consensus version ahead of the production table.
...
- **Transaction building and broadcasting**: LionDen routes execution through `@provablehq/sdk` APIs (`ProgramManager`, devnode transaction builders, broadcast helpers), using the TypeScript SDK facade backed by WASM internals, and defaults to the same for deploy/upgrade. Doko-js shells out to the Leo CLI for execution/deploy flows, which keeps it close to CLI behavior but requires assembling command-line arguments and parsing process output/errors for transaction IDs, results, and failures. LionDen can shell out too — `deploy.backend: "leo"` — but by choice and only for deploy/upgrade, where the SDK's monolithic WASM key synthesis has a hard memory ceiling. It avoids doko-js's output-parsing problem by having Leo write the transaction to a `--save` directory rather than broadcast it: LionDen submits those bytes itself, so the transaction id is the broadcast response and the chain's verdict comes from LionDen's own confirmation polling. Nothing is scraped from stdout.
...
- [`architecture.md`](architecture.md), [`compiler.md`](compiler.md), [`network.md`](network.md), [`deployment.md`](deployment.md), [`deploy-backends.md`](deploy-backends.md), [`testing.md`](testing.md) — subsystem deep dives.
