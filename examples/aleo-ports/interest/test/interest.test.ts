...
describe("interest.aleo", () => {
  ...
  it("fixed_iteration_interest compounds 10 rounds", async () => {
    expect(await interest.fixed_iteration_interest.locally({ arg0: 100, arg1: 10 })).toBe(256);
  });

  // Zero rate is a no-op.
  it("fixed_iteration_interest is a no-op at 0% rate", async () => {
    expect(await interest.fixed_iteration_interest.locally({ arg0: 100, arg1: 0 })).toBe(100);
  });
  ...
  it("bounded_iteration_interest with 0 iterations returns capital", async () => {
    expect(
      await interest.bounded_iteration_interest.locally({
        arg0: 100,
        arg1: 10,
        arg2: 0,
      }),
