...
describe("interest.aleo", () => {
  ...
  it("fixed_iteration_interest compounds 10 rounds", async () => {
    expect(await interest.fixed_iteration_interest.locally(100, 10)).toBe(256);
  });

  // Zero rate is a no-op.
  it("fixed_iteration_interest is a no-op at 0% rate", async () => {
    expect(await interest.fixed_iteration_interest.locally(100, 0)).toBe(100);
  });
  ...
  it("bounded_iteration_interest with 0 iterations returns capital", async () => {
    expect(await interest.bounded_iteration_interest.locally(100, 10, 0)).toBe(100);
  });
  ...
  it("bounded_iteration_interest matches fixed at 10 iterations", async () => {
    expect(await interest.bounded_iteration_interest.locally(100, 10, 10)).toBe(256);
  });
});
