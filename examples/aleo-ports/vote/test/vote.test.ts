...
//   - *Broadcast() → fires finalize, has side effects
...
describe("vote.aleo", () => {
  ...
  it("propose() registers a new proposal and initializes its ticket count", async () => {
    const info = {
      title: "11111111field",
      content: "22222222field",
      proposer: proposer(),
    };

    // Local: capture the typed Proposal record so we can read pid.
    const [proposal] = await vote.withSigner(ctx!.accounts[0]!).propose(info);
    pid = proposal.id;

    // Broadcast: actually fire the finalize so tickets[pid] is set to 0.
    await vote.withSigner(ctx!.accounts[0]!).proposeBroadcast(info);
  ...
  it("new_ticket() increments tickets[pid]", async () => {
    ...
    await vote.new_ticketBroadcast(pid!, voter());
  ...
  it("agree() increments agree_votes[pid]", async () => {
    ...
    const [ticket] = await vote.new_ticket(pid!, voter());

    await vote.withSigner(ctx!.accounts[1]!).agreeBroadcast(ticket);
  ...
  it("disagree() increments disagree_votes[pid]", async () => {
    ...
    await vote.withSigner(ctx!.accounts[1]!).disagreeBroadcast(ticket);
