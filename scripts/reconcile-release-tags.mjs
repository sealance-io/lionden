...
const DEFAULT_REGISTRY_TIMEOUT_MS = 5 * 60_000;
const DEFAULT_REGISTRY_REQUEST_TIMEOUT_MS = 15_000;
...
async function fetchPublishedRelease(name, version, registryUrl, fetchImpl, requestTimeoutMs) {
  ...
  if (!release) {
    throw new RetryableRegistryError(`${name}@${version} is not visible in the npm packument yet`);
  }
...
async function publishedRelease(
  ...
  registryUrl,
  fetchImpl,
  { deadlineMs, requestTimeoutMs, retryDelayMs, sleepImpl, nowImpl },
) {
  let lastError;
  let attempts = 0;
  while (nowImpl() < deadlineMs) {
    attempts++;
    try {
      const remainingMs = Math.max(1, deadlineMs - nowImpl());
      return await fetchPublishedRelease(
        name,
        version,
        registryUrl,
        fetchImpl,
      ...
      const delayMs = Math.min(
        retryDelayMs * 2 ** Math.min(attempts - 1, 30),
        MAX_REGISTRY_RETRY_DELAY_MS,
        remainingMs,
      );
      console.warn(`${error.message}; retrying npm metadata in ${delayMs}ms`);
      await sleepImpl(delayMs);
    }
  }

  throw new Error(
    `npm metadata for ${name}@${version} was not ready before the shared registry deadline ` +
      `after ${attempts} attempt${attempts === 1 ? "" : "s"}: ${lastError?.message ?? "deadline expired"}`,
    { cause: lastError },
  );
}
...
export async function reconcileReleaseTags({
  ...
  registryRequestTimeoutMs = DEFAULT_REGISTRY_REQUEST_TIMEOUT_MS,
  ...
  if (!Number.isFinite(registryTimeoutMs) || registryTimeoutMs <= 0) {
    throw new Error("registryTimeoutMs must be a positive number");
  }
  if (!Number.isFinite(registryRequestTimeoutMs) || registryRequestTimeoutMs <= 0) {
    throw new Error("registryRequestTimeoutMs must be a positive number");
  }
  if (!Number.isFinite(registryRetryDelayMs) || registryRetryDelayMs < 0) {
  ...
  const registryDeadlineMs = nowImpl() + registryTimeoutMs;
  const releases = await Promise.all(
    packages.map(({ manifest }) =>
      publishedRelease(manifest.name, manifest.version, registryUrl, fetchImpl, {
